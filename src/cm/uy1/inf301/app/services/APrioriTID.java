...
public class APrioriTID
{
    ...
    protected ArrayList<ArrayList<Itemset>> TIDCandidates;
    ...
    public ArrayList<ArrayList<Itemset>> TIDPass() {
        final ArrayList<ArrayList<Itemset>> newSetOfCandidates = new ArrayList<ArrayList<Itemset>>();
        for (final ArrayList<Itemset> TIDCandidate : this.TIDCandidates) {
            final ArrayList<Itemset> transactionCandidates = new ArrayList<Itemset>();
            for (final Itemset candidate : this.candidates) {
                Itemset copy = new Itemset(candidate);
                copy.removeAt(copy.size() - 1);
                if (copy.isIn(TIDCandidate)) {
                    copy = new Itemset(candidate);
                    copy.removeAt(copy.size() - 2);
                    if (!copy.isIn(TIDCandidate)) {
                        continue;
                    }
                    transactionCandidates.add(candidate);
                }
            }
            for (final Itemset candidate : this.candidates) {
                if (candidate.isIn(transactionCandidates)) {
                    candidate.increment();
                }
            }
            if (transactionCandidates.isEmpty()) {
                newSetOfCandidates.add(transactionCandidates);
            }
        }
        return newSetOfCandidates;
    }
    
    public void aPrioriTID() {
        ...
        while (!this.currentFrequentItemsets.isEmpty()) {
            ...
            this.TIDCandidates = new ArrayList<ArrayList<Itemset>>(this.TIDPass());
            this.eliminateCandidates();
        }
